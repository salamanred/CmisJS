...
describe('CmisJS library test', function () {
  ...
  it('should return object children', function (done) {
  	session.getChildren(randomFolderId).ok(function (data) {
  		assert(
  			data.objects[0].object.succinctProperties['cmis:name']=='First Level'
  ...
  it('should return object descendants', function (done) {
  	session.getDescendants(randomFolderId).ok(function (data) {
  		assert(
  			data[0].object.object.succinctProperties['cmis:name']=='First Level'
  			, "Should have a child named 'First Level'");
  		assert(
  			data[0].children[0].object.object.succinctProperties['cmis:name']=='Second Level'
  ...
  it('should delete object content', function (done) {
    session.deleteContentStream(docId, {changeToken:changeToken}).ok(function (data) {
      assert(data,'OK');
      done();
    });
  });
